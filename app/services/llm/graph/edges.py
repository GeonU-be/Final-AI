...
workflow.add_node("keyword_join_node", keyword_join_node)
...
workflow.add_conditional_edges(
    ...
    {"need_keyword": "crawling_keywords_node", "has_keyword": "keyword_join_node"},
)

workflow.add_edge("crawling_keywords_node", "make_keyword_node")
workflow.add_edge("make_keyword_node", "keyword_join_node")

# 키워드를 확보한 뒤 각 판매처 크롤링을 동시에 진행하고 필터링 노드에서 조인한다.
workflow.add_edge("keyword_join_node", "crawling_items_ssadagu_node")
workflow.add_edge("keyword_join_node", "crawling_items_coupang_node")
...
workflow.add_conditional_edges(
    ...
    {"done": "generate_ads", "retry": "keyword_join_node", "stop": "job_failed"},
)
