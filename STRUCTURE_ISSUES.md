...
- 구성 파일 `app/config.py`가 비어 있어 환경 변수/상수 관리가 없고, 코드 곳곳에서 `os.getenv`를 직접 호출해 일관성·기본값 관리가 어려운 상태입니다.
...
- config 정리: `os.getenv` 호출을 중앙집중화할지 결정 필요. 옵션 1) `app/config.py`에서 모든 환경변수 기본값/타입을 정의해 일관성 확보, 옵션 2) 현행 분산 호출 유지. 운영/테스트 환경 분기와 기본값 관리가 필요하면 옵션 1을 권장(미결).
